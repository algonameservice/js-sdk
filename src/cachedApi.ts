...
export default class CachedApi {
  ...
  protected ESCROW: string = TESTNET_ESCROW;
  protected APP: number = TESTNET_APP_ID;
  ...
  protected async getTeal(name: string): Promise<LogicSigAccount> {
    ...
    let program = await this.rpc.compile(generateTeal(name as string, this.ESCROW, this.APP)).do();
